...
async def _deserialize_echo_message(http_response: HTTPResponse, config: Config) -> EchoMessageOutput:
...
async def _deserialize_error_echo_message(http_response: HTTPResponse, config: Config) -> ApiError[Any]:
